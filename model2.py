...
class QRSLightning(pl.LightningModule):
    ...
    def on_test_epoch_end(self):
        ...
        metrics = {"Loss": [avg_loss], "Accuracy": [acc], "Precision": [prec], "Recall": [rec], "F1 Score": [f1_val], "ROC AUC": [roc_auc], "Dice": [d]}
...
if __name__ == "__main__":
    ...
    random.seed(42)
    output_dir = "output"
    train_dataset = ECGDataset(os.path.join(output_dir, "train_signals.npy"), os.path.join(output_dir, "train_masks.npy"))
    val_dataset = ECGDataset(os.path.join(output_dir, "val_signals.npy"), os.path.join(output_dir, "val_masks.npy"))
    test_dataset = ECGDataset(os.path.join(output_dir, "test_signals.npy"), os.path.join(output_dir, "test_masks.npy"))
    ...
    trainer = pl.Trainer(max_epochs=50, accelerator="cuda" if torch.cuda.is_available() else "cpu", devices=1, callbacks=[early_stop, checkpoint])
